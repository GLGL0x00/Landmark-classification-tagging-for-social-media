...
def get_data_loaders(
    ...
    data_transforms = {
        "train": transforms.Compose(
            ...
            #RandomResizedCrop: Simulates zooming in on different parts 
            transforms.RandomResizedCrop(size=(224, 224), scale=(0.8, 1.0)),
            #RandomAffine: to handle different angles, distances, and positions.
            transforms.RandomAffine(degrees=10, translate=(0.1, 0.1), scale=(0.9, 1.1), shear=5),
            #ColorJitter: to handle lighting conditions, camera settings, and color profiles
            transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1),
            #RandomHorizontalFlip: Helps in handling cases where landmarks appear symmetrically or when users post mirrored images.
            transforms.RandomHorizontalFlip(p=0.5),
            #GaussianBlur: Simulates out-of-focus images,
            transforms.GaussianBlur(kernel_size=5, sigma=(0.1, 2.0)),
            # transforms it to a float tensor in the range 0.0 - 1.0
            transforms.ToTensor(),
            #renormalizes the tensor to be between -1.0 and 1.0 
